...
```py title="finding element position"
colors = ['red', 'green', 'blue', 'yellow', 'purple']

print(colors.index('blue')) # 2
print(colors.index('orange')) # ValueError: 'orange' is not in list
```

```py title="removing element"
colors = ['red', 'green', 'blue', 'yellow', 'purple']
...
```py title="zipping and unpacking"
colors = ('red', 'green', 'blue')
sizes = ('S', 'M', 'L')
...
```py title="items and in operator"
colors = {'red': 1, 'green': 2, 'blue': 3}
...
# color1 elements not in colors2

colors2.difference(colors1)
# color2 elements not in colors1
```
...
- created by `namedtuple(typename, field_names_list)`
